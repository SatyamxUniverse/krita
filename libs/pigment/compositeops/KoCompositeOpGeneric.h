...
/**
 * Generic CompositeOp for separable channel compositing functions, with
 * "Wet" alpha blending.
 *
 * A template to generate a KoCompositeOp class by just specifying a
 * blending/compositing function. This template works with compositing functions
 * for separable channels (means each channel of a pixel can be processed separately).
 *
 * The alpha channel is calculated using "Wet" formula to blend source and
 * destination Alppha channels smoothly.
 *
 * This template is based on KoCompositeOpGenericSC but code for handling
 * alpha channel is changed to use max() formula.
 */
template<class Traits,
         typename Traits::channels_type compositeFunc(typename Traits::channels_type, typename Traits::channels_type)>
class KoCompositeOpGenericSCWet : public KoCompositeOpBase<Traits, KoCompositeOpGenericSCWet<Traits, compositeFunc>>
{
    typedef KoCompositeOpBase<Traits, KoCompositeOpGenericSCWet<Traits, compositeFunc>> base_class;
    typedef typename Traits::channels_type channels_type;
...
public:
    KoCompositeOpGenericSCWet(const KoColorSpace *cs, const QString &id, const QString &category)
        : base_class(cs, id, category)
    {
    }
...
public:
                                                     ...
                                                     const QBitArray &channelFlags)
    {
        ...
        /******************************************************************
         * Calculate color channels using compositing functions. (cfName) *
         ******************************************************************/

        // Check KoCompositeOpFunctions.h for compositing functions themselves.
        ...
        if (alphaLocked) {
            ...
            if (newDstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i))) {
                        channels_type result = blend(src[i],
                                                     srcAlpha,
                                                     dst[i],
                                                     dstAlpha,
                                                     compositeFunc(src[i],dst[i])
                                                     );
                        dst[i] = div(result, newDstAlpha);
                    }
                }
            ...
            /*********************************************
             * Calculate and return blended (Wet) Alpha. *
             *********************************************/

            /*
             * Uses max(d, s) for simple union of Alpha channels.
             * Standard blending would be:
             * newDstAlpha = unionShapeOpacity(srcAlpha, dstAlpha)
            */

            newDstAlpha = qMax(srcAlpha, dstAlpha);
...
/**
 * Generic CompositeOp for nonseparable/HSX channels compositing functions, with
 * "Wet" alpha channel blending.
 *
 * A template to generate a KoCompositeOp class by just specifying a
 * blending/compositing function. This template works with compositing functions
 * for RGB channels only (the channels can not be processed separately)
 ...
 * destination Alppha channels smoothly.
 *
 * This template is based on KoCompositeOpGenericHSL but code for handling
 * alpha channel is changed to use max() formula.
 */
template<class Traits, void compositeFunc(float, float, float, float &, float &, float &)>
class KoCompositeOpGenericHSXWet : public KoCompositeOpBase<Traits, KoCompositeOpGenericHSXWet<Traits, compositeFunc>>
{
    typedef KoCompositeOpBase<Traits, KoCompositeOpGenericHSXWet<Traits, compositeFunc>> base_class;
    typedef typename Traits::channels_type channels_type;
...
public:
    KoCompositeOpGenericHSXWet(const KoColorSpace *cs, const QString &id, const QString &category)
        : base_class(cs, id, category)
    {
    }
                                                     ...
                                                     const QBitArray &channelFlags)
    {
        ...
        if (alphaLocked) {
            ...
            if (newDstAlpha != zeroValue<channels_type>()) {
                ...
                if (allChannelFlags || channelFlags.testBit(red_pos))
                    dst[red_pos] =
                        div(blend(src[red_pos], srcAlpha, dst[red_pos], dstAlpha, scale<channels_type>(dstR)),
                            newDstAlpha);

                if (allChannelFlags || channelFlags.testBit(green_pos))
                    dst[green_pos] =
                        div(blend(src[green_pos], srcAlpha, dst[green_pos], dstAlpha, scale<channels_type>(dstG)),
                            newDstAlpha);

                if (allChannelFlags || channelFlags.testBit(blue_pos))
                    dst[blue_pos] =
                        div(blend(src[blue_pos], srcAlpha, dst[blue_pos], dstAlpha, scale<channels_type>(dstB)),
                            newDstAlpha);
            }
 ...
 * destination Alppha channels smoothly.
 *
 * This template is based on KoCompositeOpGenericSCAlpha but code for handling
 * alpha channel is changed to use max() formula.
 */
template<class Traits, void compositeFunc(float, float, float &, float &)>
class KoCompositeOpGenericSCAlphaWet
    : public KoCompositeOpBase<Traits, KoCompositeOpGenericSCAlphaWet<Traits, compositeFunc>>
{
    typedef KoCompositeOpBase<Traits, KoCompositeOpGenericSCAlphaWet<Traits, compositeFunc>> base_class;
    typedef typename Traits::channels_type channels_type;
...
public:
    KoCompositeOpGenericSCAlphaWet(const KoColorSpace *cs, const QString &id, const QString &category)
        : base_class(cs, id, category)
    {
    }
