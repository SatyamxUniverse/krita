#ifndef KOCOMPOSITEOPWETSCREEN_H
#define KOCOMPOSITEOPWETSCREEN_H
...
class KoCompositeOpWetScreen : public KoCompositeOpBase<CS_Traits, KoCompositeOpWetScreen<CS_Traits>>
{
    typedef KoCompositeOpBase<CS_Traits, KoCompositeOpWetScreen<CS_Traits>> base_class;
...
public:
    KoCompositeOpWetScreen(const KoColorSpace *cs)
        : base_class(cs, COMPOSITE_WET_SCREEN, KoCompositeOp::categoryMix())
    {
    }
            ...
            if (dstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i)))
                        dst[i] = lerp(dst[i], cfScreen(src[i], dst[i]), srcAlpha);
                }
            }
            ...
            if (newDstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i))) {
                        ...
                        channels_type result = blend(src[i], srcAlpha, dst[i], dstAlpha, cfScreen(src[i], dst[i]));
...
#endif // KOCOMPOSITEOPWETSCREEN_H
