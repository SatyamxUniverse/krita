#ifndef KOCOMPOSITEOPWETPARALLEL_H
#define KOCOMPOSITEOPWETPARALLEL_H
...
class KoCompositeOpWetParallel : public KoCompositeOpBase<CS_Traits, KoCompositeOpWetParallel<CS_Traits>>
{
    typedef KoCompositeOpBase<CS_Traits, KoCompositeOpWetParallel<CS_Traits>> base_class;
...
public:
    KoCompositeOpWetParallel(const KoColorSpace *cs)
        : base_class(cs, COMPOSITE_WET_PARALLEL, KoCompositeOp::categoryMix())
    {
    }
            ...
            if (dstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i)))
                        dst[i] = lerp(dst[i], cfParallel(src[i], dst[i]), srcAlpha);
                }
            }
            ...
            if (newDstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i))) {
                        ...
                        channels_type result = blend(src[i], srcAlpha, dst[i], dstAlpha, cfParallel(src[i], dst[i]));
...
#endif // KOCOMPOSITEOPWETPARALLEL_H
