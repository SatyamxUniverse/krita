#ifndef KOCOMPOSITEOPWETCOLORDODGE_H
#define KOCOMPOSITEOPWETCOLORDODGE_H
...
class KoCompositeOpWetColorDodge : public KoCompositeOpBase<CS_Traits, KoCompositeOpWetColorDodge<CS_Traits>>
{
    typedef KoCompositeOpBase<CS_Traits, KoCompositeOpWetColorDodge<CS_Traits>> base_class;
...
public:
    KoCompositeOpWetColorDodge(const KoColorSpace *cs)
        : base_class(cs, COMPOSITE_WET_COLOR_DODGE, KoCompositeOp::categoryMix())
    {
    }
            ...
            if (dstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i)))
                        dst[i] = lerp(dst[i], cfColorDodge(src[i], dst[i]), srcAlpha);
                }
            }
            ...
            if (newDstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i))) {
                        ...
                        channels_type result = blend(src[i], srcAlpha, dst[i], dstAlpha, cfColorDodge(src[i], dst[i]));
...
#endif // KOCOMPOSITEOPWETCOLORDODGE_H
