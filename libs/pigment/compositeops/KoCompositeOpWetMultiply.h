#ifndef KOCOMPOSITEOPWETMULTIPLY_H
#define KOCOMPOSITEOPWETMULTIPLY_H
...
class KoCompositeOpWetMultiply : public KoCompositeOpBase<CS_Traits, KoCompositeOpWetMultiply<CS_Traits>>
{
    typedef KoCompositeOpBase<CS_Traits, KoCompositeOpWetMultiply<CS_Traits>> base_class;
...
public:
    KoCompositeOpWetMultiply(const KoColorSpace *cs)
        : base_class(cs, COMPOSITE_WET_MULTIPLY, KoCompositeOp::categoryMix())
    {
    }
            ...
            if (dstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i)))
                        dst[i] = lerp(dst[i], cfMultiply(src[i], dst[i]), srcAlpha);
                }
            }
            ...
            if (newDstAlpha != zeroValue<channels_type>()) {
                for (qint32 i = 0; i < channels_nb; i++) {
                    if (i != alpha_pos && (allChannelFlags || channelFlags.testBit(i))) {
                        ...
                        channels_type result = blend(src[i], srcAlpha, dst[i], dstAlpha, cfMultiply(src[i], dst[i]));
...
#endif // KOCOMPOSITEOPWETMULTIPLY_H
