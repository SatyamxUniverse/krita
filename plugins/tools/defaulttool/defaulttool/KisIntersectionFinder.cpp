...
BuildingBlock::BuildingBlock(int id, BuildingBlockElementType buildingblocktype,
                             QPointF p1)
    : ID(id), type(buildingblocktype), moveToPoint(p1) {}


BuildingBlock::BuildingBlock(int id, BuildingBlockElementType buildingblocktype,
                             Line l1)
    : ID(id), type(buildingblocktype), l(l1) {}


BuildingBlock::BuildingBlock(int id, BuildingBlockElementType buildingblocktype,
                             CubicBezier c1)
    : ID(id), type(buildingblocktype) {
...
KisClippingVertex::KisClippingVertex(QPointF pt, VertexType type, int idFirst,
                                     int idSecond, qreal para1, qreal para2)
    : point(pt), type(type), idFirstBuildingBlock(idFirst),
      idSecondBuildingBlock(idSecond), parameterFirstBuildingBlock(para1),
 ...
 */
<<<<<<< HEAD
KisIntersectionFinder::KisIntersectionFinder(QPainterPath subject,
                                             QPainterPath clip) {
  ...
  if (!subject.elementAt(0).isMoveTo()) {
    ...
    subjectVertices << KisClippingVertex(
        QPointF(0, 0), KisClippingVertex::VertexType::regularVertex);
  }
  ...
  for (int i = 0; i < subject.elementCount(); i++) {
    ...
    if (e.isMoveTo()) {
      ...
      subjectVertices << KisClippingVertex(
          currPoint, KisClippingVertex::VertexType::regularVertex);
    ...
    // hence only cubic curves with four control points are expected
      ...
      subjectVertices << KisClippingVertex(
          cp4, KisClippingVertex::VertexType::regularVertex);
  ...
  subjectVertices << KisClippingVertex(
      subjectVertices.first().point,
      KisClippingVertex::VertexType::regularVertex);
  ...
  if (!clip.elementAt(0).isMoveTo()) {
    ...
    clipVertices << KisClippingVertex(
        QPointF(0, 0), KisClippingVertex::VertexType::regularVertex);
  }
  ...
  clipVertices << KisClippingVertex(
      clipVertices.first().point, KisClippingVertex::VertexType::regularVertex);
...
bool KisIntersectionFinder::linesIntersect(const QLineF &a,
...
QVector<KisClippingVertex>
KisIntersectionFinder::intersectionPoint(CubicBezier c1, CubicBezier c2) {
  ...
  if (!c1.boundingBox().intersects(c2.boundingBox())) {
    ...
    // intersect
    if (!c1.boundingBox().contains(c2.boundingBox()) &&
        !c2.boundingBox().contains(c1.boundingBox())) {
      return res; // ult;
    }
  }
  ...
  Q_FOREACH (qreal root, roots) {
    ...
    if (supposedParameter <= 1 && supposedParameter >= 0) {
      ...
      if (!comparePoints(c1.cp0, ptOfIntersection) &&
          !comparePoints(c1.cp3, ptOfIntersection) &&
          !comparePoints(ptOfIntersection, c2.cp0) &&
        ...
        res.push_back(KisClippingVertex(ptOfIntersection,
                                        KisClippingVertex::regularIntersection,
                                        -1, -1, root, supposedParameter));
      }
    }
  }
...
QVector<KisClippingVertex>
KisIntersectionFinder::intersectionPoint(Line l1, CubicBezier c2) {
  QVector<QPointF> result;
  QVector<KisClippingVertex> res;
  ...
  if (curveBoundingBox.contains(qLine.p1()) ||
      curveBoundingBox.contains(qLine.p2())) {
    ...
    QLineF rightEdge{curveBoundingBox.topRight(),
                     curveBoundingBox.bottomRight()};
    QLineF bottomEdge{curveBoundingBox.bottomRight(),
                      curveBoundingBox.bottomLeft()};
    QLineF leftEdge{curveBoundingBox.bottomLeft(), curveBoundingBox.topLeft()};

    if (linesIntersect(qLine, topEdge) || linesIntersect(qLine, rightEdge) ||
        linesIntersect(qLine, bottomEdge) || linesIntersect(qLine, leftEdge)) {
  ...
  Q_FOREACH (qreal root, roots) {
    QPointF currIntersectionPoint{c2.getParametricX().evaluate(root),
                                  c2.getParametricY().evaluate(root)};

    if (l1.checkIntersection(currIntersectionPoint)) {

      if (!comparePoints(currIntersectionPoint, c2.cp0) &&
          !comparePoints(currIntersectionPoint, c2.cp3) &&
          !comparePoints(currIntersectionPoint, qLine.p1()) &&
          !comparePoints(currIntersectionPoint, qLine.p2())) {

        // if the intersection point lies too close to the end points of both
        // curves, reject it
        result.push_back(currIntersectionPoint);

        res.push_back(KisClippingVertex(currIntersectionPoint,
                                        KisClippingVertex::regularIntersection,
                                        -1, -1, root, root));
      }
    }
...
QVector<KisClippingVertex>
KisIntersectionFinder::intersectionPoint(CubicBezier c1, Line l2) {
...
QVector<KisClippingVertex>
KisIntersectionFinder::findIntersectionPoints(BuildingBlock e1,
                                              BuildingBlock e2) {
  ...
  if (e1.isCurveTo()) {
    ...
    if (e2.isCurveTo()) {
      ...
      Q_FOREACH (QPointF p, intersectionPoints) {
        res.append(
            KisClippingVertex(p, KisClippingVertex::regularIntersection));
      }
    }
  }
...
<<<<<<< HEAD
  ...
  Q_FOREACH (BuildingBlock b, subjectShape) {
    QVector<KisClippingVertex> intersections =
        subToClipIntersectionVertices.at(b.ID);

    if (b.isMoveTo()) {
      BuildingBlock nb(newId++, BuildingBlockElementType::MoveToElement,
                       b.getMoveTo());
      ...
      if (b.getCurveTo().cp0 != lastPt) {
        ...
        subjectVerticesWithIntersections << KisClippingVertex(
            b.getCurveTo().cp0, KisClippingVertex::regularVertex);
      }
      ...
      while (points.size()) {
        ...
        QVector<CubicBezier> tempRes = res.takeLast().splitCurve(
            currIntPoint.parameter, currIntPoint.point);
        res.append(tempRes);
      }

      Q_FOREACH (CubicBezier cb, res) {

        subjectShapeProcessed.append(BuildingBlock(
            newId++, BuildingBlockElementType::CurveToElement, cb));
        clippedCurves.append(cb);
      }

      lastPt = b.getCurveTo().cp3;
      subjectVerticesWithIntersections
          << KisClippingVertex(lastPt, KisClippingVertex::regularVertex);
      ...
      Q_FOREACH (Line ln, res) {
        ...
        subjectShapeProcessed.append(BuildingBlock(
            newId++, BuildingBlockElementType::LineToElement, ln));
      }
  ...
  Q_FOREACH (BuildingBlock b, clipShape) {
    QVector<KisClippingVertex> intersections =
        clipToSubIntersectionVertices.at(b.ID);
    if (3000000) {
      ...
      if (b.isMoveTo()) {
        ...
        while (points.size()) {
          ...
          QVector<CubicBezier> tempRes =
              res.takeLast().splitCurve(currPoint.parameter, currPoint.point);
          res.append(tempRes);
        }

        Q_FOREACH (CubicBezier cb, res) {

          clipShapeProcessed.append(BuildingBlock(
              newId++, BuildingBlockElementType::CurveToElement, cb));
          clippedCurves.append(cb);
        }
        lastPt = b.getCurveTo().cp3;
...
QPainterPath KisIntersectionFinder::subjectShapeToPath() {
  ...
  if (firstBuildingBlock.isMoveTo()) {
    ...
    netRegularVertices << firstBuildingBlock.getLineTo().getQLine().p1()
                       << firstBuildingBlock.getLineTo().getQLine().p2();
  }
    ...
    QBezier bezier =
        QBezier::fromPoints(cbFirst.cp0, cbFirst.cp1, cbFirst.cp2, cbFirst.cp3);
  ...
  for (int i = 1; i < subjectShapeProcessed.size(); i++) {
    ...
    if (subjectShapeProcessed[i].isMoveTo()) {
      ...
      netRegularVertices
          << subjectShapeProcessed[i].getLineTo().getQLine().p2();
...
<<<<<<< HEAD
  auto ip = std::unique(allCurveEndPoints.begin(),
                        allCurveEndPoints.begin() + allCurveEndPoints.size());
...
<<<<<<< HEAD
  ...
  for (int j = 0; j < path.elementCount(); j++) {
    ...
    if (endPts.size() == 2) {
      ...
      Q_FOREACH (CubicBezierData cbd, allCurveData) {
        ...
        ///std::endl;
        bool curveForward =
            (cbd.cp0 == endPts.at(1)) && (cbd.cp3 == endPts.at(0));
        bool curveBackward =
            (cbd.cp3 == endPts.at(1)) && (cbd.cp0 == endPts.at(0));
  ...
  //    if (substitutedRes.elementAt(0).type == QPainterPath::MoveToElement &&
  //    substitutedRes.elementAt(0) == QPointF(0,0)) {
  ...
  for (int j = 1; j < substitutedRes.elementCount(); j++) {
    ...
    if (ele.isMoveTo()) {
      ...
      processedRes.cubicTo(ele, substitutedRes.elementAt(j + 1),
                           substitutedRes.elementAt(j + 2));
      j += 2;
    }
  }
  //    }
