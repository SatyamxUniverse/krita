...
Term::Term(qreal coef, int x_ind, int y_ind)
    : coefficient(coef), xIndex(x_ind), yIndex(y_ind) {}

Term::Term() {}

Term::~Term() {}

qreal Term::getCoefficient() { return coefficient; }

int Term::getXIndex() { return xIndex; }

int Term::getYIndex() { return yIndex; }

void Term::addCoefficient(qreal coeff) { this->coefficient += coeff; };

void Term::setCoefficient(qreal coeff) { this->coefficient = coeff; };

void Term::setXIndex(int xIndex_arg) { this->xIndex = xIndex_arg; };

void Term::setYIndex(int YIndex_arg) { this->yIndex = YIndex_arg; };
...
bool Term::operator!=(Term &t2) {
  return (this->coefficient != t2.coefficient || this->xIndex != t2.xIndex ||
          this->yIndex != t2.yIndex);
}

bool Term::operator==(const Term &t2) {
  return (coefficient == t2.coefficient && xIndex == t2.xIndex &&
          yIndex == t2.yIndex);
}
...
Expression::Expression() { express.clear(); }
...
Expression Expression::operator*(qreal multiplying_factor) {
  Expression result;
  Q_FOREACH (Term i, this->express) {
    Term resTerm(i.getCoefficient() * multiplying_factor, i.getXIndex(),
                 i.getYIndex());
    result.add(resTerm);
  }
...
Expression Expression::operator+(Expression ex) {
  ...
  Q_FOREACH (Term i, express) { result.add(i); }
  result.add(ex);
...
void Expression::add(Term t) {
  ...
  for (int i = 0; i < this->express.size(); i++) {
    ...
    if ((curr.getXIndex() == t.getXIndex()) &&
        (curr.getYIndex() == t.getYIndex())) {
...
void Expression::add(Expression ex) {
  Q_FOREACH (Term i, ex.express) { this->add(i); }
}
...
void Expression::subtract(Expression ex) {
  ...
  Q_FOREACH (Term i, ex.express) { this->subtract(i); }
}

QVector<qreal> Expression::coeffs() {
  QVector<qreal> result;
  Q_FOREACH (Term i, express) { result.push_back(i.getCoefficient()); }
  return result;
}
...
qreal Expression::evaluate2d(qreal x, qreal y) {
  ...
  Q_FOREACH (Term i, express) {
    result +=
        i.getCoefficient() * pow(x, i.getXIndex()) * pow(y, i.getYIndex());
  }
...
QVector<Term> Expression::getExpression() { return this->express; }
...
Term::Term (qreal coef, int x_ind, int y_ind) : coefficient (coef),
    xIndex (x_ind), yIndex (y_ind)
    {
    }
...
<<<<<<< HEAD
  std::sort(this->express.begin(), this->express.end(),
            (order == ascending ? greaterThan : smallerThan));
}
...
<<<<<<< HEAD
CubicBezier::CubicBezier(QPointF &cp_0, QPointF &cp_1, QPointF &cp_2,
                         QPointF &cp_3)
    : cp0(cp_0), cp1(cp_1), cp2(cp_2), cp3(cp_3) {
...
CubicBezier::CubicBezier(QPainterPath::Element curve) { cp0 = curve; }
...
QVector<CubicBezier> CubicBezier::splitCurve(qreal parameter) {
  ...
  QPointF pointAtT(parametric_x.evaluate(parameter),
                   parametric_y.evaluate(parameter));
...
Expression CubicBezier::element_entry(int first_pt, int second_pt) {
  ...
  int combinatorial_factor =
      (combinatorial(3, first_pt) * combinatorial(3, second_pt));
...
Expression CubicBezier::getImplicitEquation() {
  ...
  Expression cofactor_00 =
      bezierMatrix[0][0] * ((bezierMatrix[1][1] * bezierMatrix[2][2]) -
                            (bezierMatrix[1][2] * bezierMatrix[2][1]));

  Expression cofactor_01 = bezierMatrix[0][1] *
                           ((bezierMatrix[1][0] * bezierMatrix[2][2]) -
                            (bezierMatrix[1][2] * bezierMatrix[2][0])) *
                           -1;

  Expression cofactor_02 =
      bezierMatrix[0][2] * ((bezierMatrix[1][0] * bezierMatrix[2][1]) -
                            (bezierMatrix[1][1] * bezierMatrix[2][0]));
...
QRectF CubicBezier::boundingBox() { return bbox; }
...
qreal CubicBezier::inversionEquationEvaluated(QPointF &p) {
  ...
  if (inversionEquationDen.getExpression().size() == 0 &&
      inversionEquationNum.getExpression().size() == 0) {
    qreal c1Num = cp0.x() * (cp1.y() - cp3.y()) -
                  cp0.y() * (cp1.x() - cp3.x()) + cp1.x() * cp3.y() -
                  cp1.y() * cp3.x();
    qreal c1Den = cp1.x() * (cp2.y() - cp3.y()) -
                  cp1.y() * (cp2.x() - cp3.x()) + cp2.x() * cp3.y() -
                  cp3.x() * cp2.y();
    qreal c1 = c1Num / (3 * c1Den);

    qreal c2Num = cp0.x() * (cp2.y() - cp3.y()) -
                  cp0.y() * (cp2.x() - cp3.x()) + cp2.x() * cp3.y() -
                  cp2.y() * cp3.x();
    qreal c2Den = c1Den;
    qreal c2 = -(c2Num / (3 * c2Den));

    Expression la =
        bezierMatrix[0][1] * c1 + bezierMatrix[1][1] * c2 + bezierMatrix[1][2];
    Expression lb =
        bezierMatrix[0][2] * c1 + bezierMatrix[1][2] * c2 + bezierMatrix[2][2];

    inversionEquationNum = lb;
    inversionEquationDen = lb - la;
  }

  qreal t = inversionEquationNum.evaluate2d(p.x(), p.y()) /
            inversionEquationDen.evaluate2d(p.x(), p.y());
...
QVector<qreal> CubicBezier::findRoots(CubicBezier &cb2) {
  ...
  //        std::cout << "cleaning and substituting param eqns in implicit: " <<
  ...
  for (int i = 0; i < numTerms; i++) {
    //        std::cout <<"root "<< i << ": "<< z[2*i]<<" "  << z[2*i +1] <<
    //        std::endl;  // << sign.toStdString()
...
QLineF Line::getQLine() { return qtLine; }
...
QVector<Line> Line::splitLine(QVector<QPointF> points) {
  ...
  for (int i = 0; i < params.size() - 1; i++) {
    ...
    res.push_back(
        Line(qtLine.pointAt(params.at(i)), qtLine.pointAt(params.at(i + 1))));
  }

  return res;
}

Expression Line::getImplicitEquation() { return implicitEquation; }
...
bool Line::checkIntersection(Line &l2) {
  ...
  // version 5.14
  QLineF::IntersectType intersect =
      this->getQLine().intersect(l2.getQLine(), intersection);
...
//    std::cout << "x: " << qAbs((splittingPoint - pointAtT).x()) << std::endl;
//    std::cout << "y: " << qAbs((splittingPoint - pointAtT).y()) << std::endl;
...
void CubicBezier::computeBoundingBox() {
    ...
    */
    ...
    // below one creates tighter boxes, but faster for arcs.
...
QVector<qreal> CubicBezier::findRoots(CubicBezier& cb2)
    ...
    auto duration = duration_cast<microseconds>( t1 - t0 );
//    std::cout << "get implicit eqn:                                 " << duration.count() << " microsecs" << std::endl;
...
//        }
        ...
        Expression e1, e2;
//        currXIndex = i.getXIndex();
//        currYIndex = i.getYIndex();
//        currCoef = i.getCoefficient();
    ...
    auto t2 = high_resolution_clock::now ();
    duration = duration_cast<microseconds>( t2 - t1);
//        std::cout << "cleaning and substituting param eqns in implicit: " << duration.count() << " microsecs" << std::endl;
...
//    std::cout << "validate leading coef:                               " << duration.count() << " microsecs" << std::endl;
...
//        std::cout << "sort:                                             " << duration.count() << " microsecs" << std::endl;
...
//        std::cout << "GSL root finding:                                 " << duration.count() << " microsecs\n\n" << "***********\n"<< std::endl;
...
<<<<<<< HEAD
  ...
  QLineF::IntersectionType intersect2 =
      l2.intersects(this->getQLine(), intersection);
  return (intersect2 == QLineF::IntersectType::BoundedIntersection) ? true
                                                                    : false;
#else
  QLineF::IntersectType intersect =
      l2.intersect(this->getQLine(), intersection);
  return (intersect == QLineF::IntersectType::BoundedIntersection) ? true
                                                                   : false;
=======
    QPointF* intersection;
    // intersect() is deprecated, however the newer function was introduced in version 5.14
...
<<<<<<< HEAD
  qreal resultant = this->getImplicitEquation().evaluate2d(PointToCheck.x(),
                                                           PointToCheck.y());
  ...
  if (qMin(qtLine.p1().x(), qtLine.p2().x()) < PointToCheck.x() + 1e-6 &&
      qMax(qtLine.p1().x(), qtLine.p2().x()) > PointToCheck.x() - 1e-6 &&
      qMin(qtLine.p1().y(), qtLine.p2().y()) < PointToCheck.y() + 1e-6 &&
      qMax(qtLine.p1().y(), qtLine.p2().y()) > PointToCheck.y() - 1e-6) {
...
Expression Line::getParametricX() { return this->parametrixX; }

Expression Line::getParametricY() { return this->parametricY; }

bool Line::isVertical() { return lineType == Vertical ? true : false; }

bool Line::isHorizontal() { return lineType == Horizontal ? true : false; }

bool Line::isOblique() { return lineType == Oblique ? true : false; }
=======
