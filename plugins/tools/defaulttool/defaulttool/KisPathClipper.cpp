...
<<<<<<< HEAD
  1. Find all intersections between the two paths (including
 self-intersections), and build a winged edge structure of non-intersecting
 parts.
  2. While there are more unhandled edges:
    ...
    5. Traverse intersections left to right deciding whether each subpath should
 be added or not.
    6. If the subpath should be added, traverse the winged-edge structure and
 add the edges to a separate winged edge structure.
    7. Mark all edges in subpaths crossing the horizontal line as handled.
 8. (Optional) Simplify the resulting winged edge structure by merging shared
 edges.
=======
...
public:
  void produceIntersections(KisPathSegments &segments);
  bool hasIntersections(const KisPathSegments &a,
                        const KisPathSegments &b) const;
...
bool QIntersectionFinder::linesIntersect(const QLineF &a,
  ...
  const qreal tp =
      (qDelta.y() * (q1.x() - p1.x()) - qDelta.x() * (q1.y() - p1.y())) *
      invPar;
  ...
  const qreal tq =
      (pDelta.y() * (q1.x() - p1.x()) - pDelta.x() * (q1.y() - p1.y())) *
      invPar;

  return tq >= 0 && tq <= 1;
}

bool QIntersectionFinder::hasIntersections(const KisPathSegments &a,
                                           const KisPathSegments &b) const {
...
class SegmentTree {
public:
  SegmentTree(KisPathSegments &segments);
...
private:
  ...
  void produceIntersections(const TreeNode &node, int segment,
                            const RectF &segmentBounds, const RectF &nodeBounds,
                            int axis);
  void intersectLines(const QLineF &a, const QLineF &b,
...
SegmentTree::SegmentTree(KisPathSegments &segments)
    : m_segments(segments), m_intersections(0) {
  m_bounds.x1 = qt_inf();
  m_bounds.y1 = qt_inf();
  m_bounds.x2 = -qt_inf();
  m_bounds.y2 = -qt_inf();
...
TreeNode SegmentTree::buildTree(int first, int last, int depth,
                                const RectF &bounds) {
...
void SegmentTree::intersectLines(const QLineF &a, const QLineF &b,
                                 QDataBuffer<QIntersection> &intersections) {
  const QPointF p1 = a.p1();
  const QPointF p2 = a.p2();
  ...
  const qreal tp =
      (qDelta.y() * (q1.x() - p1.x()) - qDelta.x() * (q1.y() - p1.y())) / par;
  const qreal tq =
      (pDelta.y() * (q1.x() - p1.x()) - pDelta.x() * (q1.y() - p1.y())) / par;
...
void SegmentTree::produceIntersectionsLeaf(const TreeNode &node, int segment) {
  ...
  for (int i = node.index.interval.first; i < node.index.interval.last; ++i) {
    ...
    //        m_intersections.reset(); // tanmay added

    for (int k = 0; k < m_intersections.size(); ++k) {
      KisPathSegments::Intersection i_isect, j_isect;
      i_isect.t = m_intersections.at(k).alphaA;
      j_isect.t = m_intersections.at(k).alphaB;

      i_isect.vertex = j_isect.vertex =
          m_segments.addPoint(m_intersections.at(k).pos);
...
void SegmentTree::produceIntersections(const TreeNode &node, int segment,
  ...
  if (segment > node.lowestLeftIndex &&
      (&segmentBounds.x1)[axis] <= node.splitLeft)
    produceIntersections(m_tree.at(node.index.children.left), segment,
                         segmentBounds, lbounds, !axis);

  if (segment > node.lowestRightIndex &&
      (&segmentBounds.x2)[axis] >= node.splitRight)
    produceIntersections(m_tree.at(node.index.children.right), segment,
                         segmentBounds, rbounds, !axis);
}

} // namespace

void QIntersectionFinder::produceIntersections(KisPathSegments &segments) {
...
public:
  ...
  QKdPointTree(const KisPathSegments &segments)
      : m_segments(&segments), m_nodes(m_segments->points()), m_id(0) {
  ...
  Node *rootNode() { return &m_nodes.at(m_rootNode); }

  inline int nextId() { return m_id++; }

private:
  const KisPathSegments *m_segments;
...
void qTraverseKdPointTree(QKdPointTree::Node &node, T &t, int depth = 0) {
  ...
  const bool traverseRight = (status == QKdPointTree::TraverseBoth ||
                              status == QKdPointTree::TraverseRight);
  const bool traverseLeft = (status == QKdPointTree::TraverseBoth ||
                             status == QKdPointTree::TraverseLeft);
...
public:
  QKdPointFinder(int point, const KisPathSegments &segments, QKdPointTree &tree)
  ...
  inline QKdPointTree::Traversal operator()(QKdPointTree::Node &node,
                                            int depth) {
    if (m_result != -1)
...
void KisPathSegments::mergePoints() {
  ...
  if (tree.rootNode()) {
    ...
    for (int i = 0; i < points(); ++i) {
      ...
      QT_PREPEND_NAMESPACE(qTraverseKdPointTree<QKdPointFinder>)
      (*tree.rootNode(), finder);
    ...
    for (int i = 0; i < m_intersections.size(); ++i)
      m_intersections.at(i).vertex =
          pointIndices.at(m_intersections.at(i).vertex);

    m_points.swap(mergedPoints);
  }
}

void KisWingedEdge::intersectAndAdd() {
  ...
  QDataBuffer<KisPathSegments::Intersection> intersections(
      m_segments.segments()); // m_segments.segments() returns size
  for (int i = 0; i < m_segments.segments(); ++i) {
    intersections.reset();

    int pathId = m_segments.pathId(
        i); // can be 0 or 1, mostly indicates subjectShape or clipShape.
    int curveId = m_segments.segmentAt(i).curveID;

    //        std::cout << "^^^^^^^^^^^^^^^^^^^" << curveId << std::endl;

    const KisPathSegments::Intersection *isect = m_segments.intersectionAt(i);
    while (isect) {
      intersections << *isect;
    ...
    std::sort(intersections.data(),
              intersections.data() + intersections.size());
    ...
    for (int j = 0; j < intersections.size(); ++j) {
      const KisPathSegments::Intersection &isect = intersections.at(j);

      KisPathEdge *ep = edge(addEdge(last, isect.vertex, curveId));

      if (ep) {
        const int dir =
            m_segments.pointAt(last).y() < m_segments.pointAt(isect.vertex).y()
                ? 1
                : -1;
        if (pathId == 0)
          ep->windingA += dir; // if upward, +1. else, -1
        else
          ep->windingB += dir; // if upward, +1. else, -1
      }
    ...
    KisPathEdge *ep = edge(addEdge(last, second, curveId));

    if (ep) {
      const int dir =
          m_segments.pointAt(last).y() < m_segments.pointAt(second).y() ? 1
                                                                        : -1;
      if (pathId == 0)
        ep->windingA += dir;
      else
...
KisWingedEdge::KisWingedEdge()
    : m_segments(0), m_edges(0), m_vertices(0)

{}

KisWingedEdge::KisWingedEdge(const QPainterPath &subject,
                             const QPainterPath &clip)
    : m_segments(subject.elementCount()), m_edges(subject.elementCount()),
      m_vertices(subject.elementCount())

{
  m_segments.setPath(subject);
  m_segments.addPath(clip); //, curves

  intersectAndAdd();
}

KisWingedEdge::TraversalStatus
KisWingedEdge::next(const KisWingedEdge::TraversalStatus &status) const {
  const KisPathEdge *sp = edge(status.edge);
  Q_ASSERT(sp);
  ...
  const KisPathEdge *rp = edge(result.edge);
  Q_ASSERT(rp);
...
static bool isLine(const QBezier &bezier) {
  ...
  return (equal_1_2 && equal_3_4) || (equal_1_2 && equal_2_3) ||
         (equal_2_3 && equal_3_4);
}

void KisPathSegments::setPath(const QPainterPath &path) {
  m_points.reset();
  m_intersections.reset();
  m_segments.reset();
...
void KisPathSegments::addPath(const QPainterPath &path) {
  ...
  for (int i = 0; i < path.elementCount(); ++i) {
    ...
    case QPainterPath::CurveToElement: {
      QBezier bezier =
          QBezier::fromPoints(m_points.at(last), path.elementAt(i),
                              path.elementAt(i + 1), path.elementAt(i + 2));
      if (isLine(bezier)) {
        m_segments << Segment(m_pathId, last, current);
      }
        ...
        int threshold = qMin<float>(64, qMax(bounds.width(), bounds.height()) *
                                            (2 * qreal(3.14) / 6));
  ...
  for (int i = 0; i < path.elementCount(); ++i) {
    ...
    case QPainterPath::CurveToElement: {
      ...
      if (isLine(bezier)) {
        ...
        //                    std::cout << ">>>>>>>>>>>>>>>>>>" << curveNumber
        //                    << std::endl;

        curves << bezier;
        std::cout << "curvelines.moveTo(QPointF(" << bezier.pt1().x() << ", "
                  << bezier.pt1().y() << "));" << std::endl;
        std::cout << "curvelines.lineTo("
                  << "QPointF(" << bezier.pt2().x() << ", " << bezier.pt2().y()
                  << "));" << std::endl; // tanmay
        std::cout << "curvelines.lineTo("
                  << "QPointF(" << bezier.pt3().x() << ", " << bezier.pt3().y()
                  << "));" << std::endl; // tanmay
        std::cout << "curvelines.lineTo("
                  << "QPointF(" << bezier.pt4().x() << ", " << bezier.pt4().y()
                  << "));" << std::endl; // tanmay
        ...
        for (int t = 1; t < threshold - 1; ++t) {
          ...
          int index = m_points.size();
          m_segments << Segment(m_pathId, last, index, curves.size() - 1);
          last = index;

          m_points << currentPoint;
        }

        m_segments << Segment(m_pathId, last, current, curves.size() - 1);
        curveNumber++;
      }
      //                m_points << currentPoint;  // tanmay
    }
      last = current;
      i += 2;
      break;
...
qreal KisWingedEdge::delta(int vertex, int a, int b) const {
  const KisPathEdge *ap = edge(a);
  const KisPathEdge *bp = edge(b);
...
KisWingedEdge::TraversalStatus KisWingedEdge::findInsertStatus(int vi,
                                                               int ei) const {
  ...
  status.traversal = KisPathEdge::RightTraversal;
  status.edge = vp->edge;

#ifdef QDEBUG_CLIPPER
  const QPathEdge *ep = edge(ei);
  qDebug() << "Finding insert status for edge" << ei << "at vertex"
           << QPointF(*vp) << ", angles: " << ep->angle << ep->invAngle;
#endif
  ...
  do {
    ...
    qDebug() << "Delta to edge" << status.edge << d2
  ...
  status.traversal = KisPathEdge::LeftTraversal;
...
#ifdef QDEBUG_CLIPPER
  qDebug() << "Inserting edge" << ei << "to"
           << (status.traversal == QPathEdge::LeftTraversal ? "left" : "right")
           << "of edge" << status.edge;
#endif
...
void KisWingedEdge::removeEdge(int ei) {
  KisPathEdge *ep = edge(ei);
  ...
  status.direction = KisPathEdge::Backward;
  ...
  edge(forwardRight.edge)
      ->setNext(forwardRight.traversal, forwardRight.direction,
                forwardLeft.edge);
  edge(forwardLeft.edge)
      ->setNext(forwardLeft.traversal, forwardLeft.direction,
                forwardRight.edge);

  edge(backwardRight.edge)
      ->setNext(backwardRight.traversal, backwardRight.direction,
                backwardLeft.edge);
  edge(backwardLeft.edge)
      ->setNext(backwardLeft.traversal, backwardLeft.direction,
                backwardRight.edge);

  ep->setNext(KisPathEdge::Forward, ei);
  ep->setNext(KisPathEdge::Backward, ei);
...
static int commonEdge(const KisWingedEdge &list, int a, int b) {
...
int KisWingedEdge::addEdge(const QPointF &a, const QPointF &b) {
  int fi = insert(a);
  int si = insert(b);
...
int KisWingedEdge::addEdge(int fi, int si,
                           int curveId) // curveId defaults to -1
{
  if (fi == si) // trivially rejected
    return -1;

  int common = commonEdge(
      *this, fi,
      si); // if is a common edge (hence already processed?), reject it
  if (common >= 0)
    return common;

  m_edges << KisPathEdge(fi, si);
  ...
  const QPointF tangent =
      QPointF(*sp) -
      QPointF(*fp); // displacement vector between first to second point

  ep->angle =
      computeAngle(tangent); // defined two functions above this one - tanmay
  ...
  KisPathEdge::Direction dirs[2] = {KisPathEdge::Backward,
                                    KisPathEdge::Forward};
  ...
  Q_ASSERT(ep->next(KisPathEdge::RightTraversal, KisPathEdge::Forward) >= 0);
  Q_ASSERT(ep->next(KisPathEdge::RightTraversal, KisPathEdge::Backward) >= 0);
  Q_ASSERT(ep->next(KisPathEdge::LeftTraversal, KisPathEdge::Forward) >= 0);
  Q_ASSERT(ep->next(KisPathEdge::LeftTraversal, KisPathEdge::Backward) >= 0);

  return ei;
}

int KisWingedEdge::insert(const QPathVertex &vertex) {
...
public:
    void produceIntersections(KisPathSegments &segments);
    bool hasIntersections(const KisPathSegments &a, const KisPathSegments &b) const;
...
bool QIntersectionFinder::hasIntersections(const KisPathSegments &a, const KisPathSegments &b) const
{
    if (a.segments() == 0 || b.segments() == 0)
        return false;
...
//    finder.produceIntersections(m_segments);
    ...
    QDataBuffer<KisPathSegments::Intersection> intersections(m_segments.segments()); // m_segments.segments() returns size
    for (int i = 0; i < m_segments.segments(); ++i) {
        intersections.reset();

        int pathId = m_segments.pathId(i);  // can be 0 or 1, mostly indicates subjectShape or clipShape.
        int curveId = m_segments.segmentAt(i).curveID;
...
KisWingedEdge::KisWingedEdge(const QPainterPath &subject, const QPainterPath &clip) :
...
KisWingedEdge::TraversalStatus KisWingedEdge::next(const KisWingedEdge::TraversalStatus &status) const
{
    const KisPathEdge *sp = edge(status.edge);
    Q_ASSERT(sp);
...
void KisPathSegments::setPath(const QPainterPath &path, QVector<QBezier> &curves)
{
    m_points.reset();
    m_intersections.reset();
    m_segments.reset();
...
void KisPathSegments::addPath(const QPainterPath &path, QVector<QBezier> &curves)
{
    int firstSegment = m_segments.size();
...
//                    std::cout << ">>>>>>>>>>>>>>>>>>" << curveNumber << std::endl;

                    curves << bezier;
                    std::cout << "curvelines.moveTo(QPointF("<< bezier.pt1().x() << ", " << bezier.pt1().y() << "));" << std::endl;
                    std::cout << "curvelines.lineTo(" <<  "QPointF(" <<
                                                   bezier.pt2().x() << ", " << bezier.pt2().y() << "));" <<std::endl; // tanmay
                    std::cout << "curvelines.lineTo(" <<  "QPointF(" <<
                                                   bezier.pt3().x() << ", " << bezier.pt3().y() << "));" <<std::endl; // tanmay
                    std::cout << "curvelines.lineTo(" <<  "QPointF(" <<
                                                   bezier.pt4().x() << ", " << bezier.pt4().y() << "));" <<std::endl; // tanmay
...
KisWingedEdge::TraversalStatus KisWingedEdge::findInsertStatus(int vi, int ei) const
{
    const QPathVertex *vp = vertex(vi);
...
int KisWingedEdge::addEdge(int fi, int si, int curveId) //curveId defaults to -1
{
    if (fi == si) // trivially rejected
        return -1;

    int common = commonEdge(*this, fi, si);  // if is a common edge (hence already processed?), reject it
    if (common >= 0)
        return common;
    ...
    const QPointF tangent = QPointF(*sp) - QPointF(*fp); // displacement vector between first to second point

    ep->angle = computeAngle(tangent); // defined two functions above this one - tanmay
    ep->invAngle = ep->angle + 64;
    ...
    QPathVertex *vertices[2] = { fp, sp };
    KisPathEdge::Direction dirs[2] = { KisPathEdge::Backward, KisPathEdge::Forward };
...
static void add(QPainterPath &path, const KisWingedEdge &list, int edge,
  ...
  //        if (status.traversal == KisPathEdge::LeftTraversal)
  //            ep->flag &= ~16;
  //        else
  //            ep->flag &= ~32;
  ...
  do {
    const KisPathEdge *ep = list.edge(status.edge);
    std::cout << "curve in addddddddddd: " << ep->curveID << std::endl;

    if (ep->curveID == -1) { // ep->curveID == -1
      ...
      int currentCurveID = ep->curveID;
      QBezier bz = list.curves.at(currentCurveID);
      ...
      do {
        ...
        status = list.next(status);
        ep = list.edge(
            status.edge); // not const, will it make a difference? - tanmay
        addLineToProxy(path, *list.vertex(ep->vertex(status.direction)));

      } while (ep->curveID == currentCurveID);

      path.moveTo(bz.pt1());
      path.cubicTo(bz.pt2(), bz.pt3(), bz.pt4());
    }
...
void KisWingedEdge::simplify() {
  for (int i = 0; i < edgeCount(); ++i) {
    ...
    //        std::cout << "curve considered for simplify: " << ep->curveID <<
    //        std::endl;
    // if both sides are part of the inside then we can collapse the edge
    int flag = 0x3 << 4;
    if ((ep->flag & flag) == flag) {
      removeEdge(i);
      std::cout << "curve edge removed: " << ep->curveID << std::endl;
      ep->flag &= ~flag;
    }
  }
}

QPainterPath KisWingedEdge::toPath() const {
  QPainterPath path;
...
bool KisPathClipper::intersect() {
  ...
  KisPathSegments a(subjectPath.elementCount());
  a.setPath(subjectPath);
  KisPathSegments b(clipPath.elementCount());
  b.setPath(clipPath);
...
KisPathClipper::KisPathClipper(const QPainterPath &subject,
                               const QPainterPath &clip)
...
static void clear(KisWingedEdge &list, int edge,
                  KisPathEdge::Traversal traversal) {
  KisWingedEdge::TraversalStatus status;
  status.edge = edge;
  status.traversal = traversal;
...
InputIterator qFuzzyFind(InputIterator first, InputIterator last, qreal val) {
  while (first != last &&
         !QT_PREPEND_NAMESPACE(qFuzzyCompare)(qreal(*first), qreal(val)))
    ++first;
  return first;
}

static bool fuzzyCompare(qreal a, qreal b) { return qFuzzyCompare(a, b); }

bool KisPathClipper::pathToRect(const QPainterPath &path, QRectF *rect) {
  if (path.elementCount() != 5)
    return false;

  const bool mightBeRect =
      path.elementAt(0).isMoveTo() && path.elementAt(1).isLineTo() &&
      path.elementAt(2).isLineTo() && path.elementAt(3).isLineTo() &&
...
QPainterPath KisPathClipper::clip(Operation operation) {
  op = operation;
  ...
  if (op != Simplify) {
    ...
    if (!clipBounds.intersects(subjectBounds)) {
      std::cout << "not intersect" << std::endl; // tanmay
      switch (op) {
      case BoolSub:
        return subjectPath;
      case BoolAnd:
      ...
      case BoolOr: {
        ...
        // result.operationInClip = "not intersect";  //tanmay
        return result;
      }
      default:
        break;
    ...
    if (clipBounds.contains(subjectBounds)) {
      std::cout << "clipBounds.contains(subjectBounds!!!!!!!!!!!!!!!!!!)"
                << std::endl; // tanmay
      if (clipIsRect) {
        switch (op) {
        case BoolSub:
        ...
        case BoolAnd:
          return subjectPath;
        case BoolOr:
          // clipPath.operationInClip = "clipbounds contains subjectbounds";
          // //tanmay
          return clipPath;
        default:
          break;
      ...
      std::cout << "subjectBounds.contains(clipBounds)" << std::endl; // tanmay
      if (subjectIsRect) {
        switch (op) {
        case BoolSub:
        ...
        case BoolAnd:
          return clipPath;
        case BoolOr:
          // subjectPath.operationInClip = "subjectbounds contains clipBounds";
          // //tanmay
          return subjectPath;
        default:
          break;
  ...
  KisWingedEdge list(subjectPath, clipPath);
...
bool KisPathClipper::doClip(KisWingedEdge &list, ClipperMode mode) {
  ...
  do {
    ...
    for (int i = 0; i < list.edgeCount(); ++i) {
      KisPathEdge *edge = list.edge(i);
      std::cout
          << "                                               curve in doclip "
          << edge->curveID << std::endl;
    ...
    if (found) {
      KisPathEdge *edge = list.edge(index);
      ...
      const int first =
          qFuzzyFind(y_coords.cbegin(), y_coords.cend(), qMin(a->y, b->y)) -
          y_coords.cbegin();
      const int last = qFuzzyFind(y_coords.cbegin() + first, y_coords.cend(),
                                  qMax(a->y, b->y)) -
                       y_coords.cbegin();
      ...
      const qreal bestY =
          0.5 * (y_coords.at(bestIdx) + y_coords.at(bestIdx + 1));
...
static void traverse(KisWingedEdge &list, int edge,
                     KisPathEdge::Traversal traversal) {
  KisWingedEdge::TraversalStatus status;
  status.edge = edge;
  status.traversal = traversal;
  ...
  do {
    int flag = status.traversal == KisPathEdge::LeftTraversal ? 1 : 2;

    KisPathEdge *ep = list.edge(status.edge);

    ep->flag |= (flag | (flag << 4));

#ifdef QDEBUG_CLIPPER
    qDebug() << "traverse: adding edge " << status.edge
             << ", mask:" << (flag << 4) << ep->flag;
#endif
...
struct QCrossingEdge {
  ...
  bool operator<(const QCrossingEdge &edge) const { return x < edge.x; }
};
Q_DECLARE_TYPEINFO(QCrossingEdge, Q_PRIMITIVE_TYPE);

static bool bool_op(bool a, bool b, KisPathClipper::Operation op) {
  switch (op) {
  case KisPathClipper::BoolAnd:
    return a && b;
  case KisPathClipper::BoolOr:
  case KisPathClipper::Simplify:
    return a || b;
  case KisPathClipper::BoolSub:
    return a && !b;
  default:
    Q_ASSERT(false);
    return false;
...
bool KisWingedEdge::isInside(qreal x, qreal y) const {
  int winding = 0;
  for (int i = 0; i < edgeCount(); ++i) {
    const KisPathEdge *ep = edge(i);
...
static QVector<QCrossingEdge> findCrossings(const KisWingedEdge &list,
                                            qreal y) {
  QVector<QCrossingEdge> crossings;
  for (int i = 0; i < list.edgeCount(); ++i) {
    const KisPathEdge *edge = list.edge(i);
...
bool KisPathClipper::handleCrossingEdges(KisWingedEdge &list, qreal y,
                                         ClipperMode mode) {
  ...
  std::sort(
      crossings.begin(),
      crossings
          .end()); // sorts all crossing edges according to their x co-ordinates
  ...
  for (int i = 0; i < crossings.size() - 1;
       ++i) { // windings nums ARE INCREMENTED INSIDE LOOP, CAN BE GREATER THAN
              // 1
    int ei = crossings.at(i).edge; // index of the edge having the crossing
    const KisPathEdge *edge = list.edge(ei);
    std::cout << "edge in handleCrossingEdges: " << edge->curveID << std::endl;

    // ******* windingA and windingB *******

    // edge winding nums are incremented (only?) in intersectAndAdd, dependent
    // on y-coords windingA is changed only if pathId of edge is 0. otherwise B
    // is changed. However this kinda doesn't make sense in the below code, as
    // windingB is related to bMask, which is clipShape's fill style.

    windingA += edge->windingA;
    windingB += edge->windingB;

    // Krita, in defaulttools.cpp, for union, used evenOdd filling. this implies
    // aMask and bMask will always be 1, and inA and inB's values are solely
    // dependent on windingA and windingB.

    // if windingA is odd, inA will be true(1). if windingA is even, inA will be
    // false(0). same for inB and windingB.
    const bool inA =
        (windingA & aMask) !=
        0; // aMask = subjectPath.fillRule() == Qt::WindingFill ? ~0x0 : 0x1;
    const bool inB = (windingB & bMask) != 0;

    const bool hasLeft = (edge->flag >> 4) & 1; // flag has 16 or 32 as value
    const bool hasRight = (edge->flag >> 4) & 2;

    const bool inside = bool_op(
        inA, inB,
        op); // decides to include or not by checking if it's inside A, inside B

    // ******* windingD *******

    windingD +=
        hasLeft ^ hasRight; // windingD will be incremented by 1 if both values
                            // are different, else will remain same

    // on the line below, the last bit has to be 1. hence, windingD is supposed
    // to be odd.
    const bool inD = (windingD & 0x1) !=
                     0; // checks if winding num D (i.e edge hasLeft or
                        // hasRight, but not both  OVER ALL CROSSINGS) is 1

    // ****** final *******
    const bool add = inD ^ inside; // inD and inside both are bools

#ifdef QDEBUG_CLIPPER
    printf("y %f, x %f, inA: %d, inB: %d, inD: %d, inside: %d, flag: %x, "
           "bezier: %p, edge: %d\n",
           y, crossings.at(i).x, inA, inB, inD, inside, edge->flag,
           edge->bezier, ei);
#endif
    ...
    if (add) {
      ...
      std::cout << "curve considered for add in handleCrossingEdges: "
                << edge->curveID << std::endl;

      if (y0 < y1) {
        if (!(edge->flag & 1))
          traverse(list, ei, KisPathEdge::LeftTraversal);

        if (!(edge->flag & 2))
          clear(list, ei, KisPathEdge::RightTraversal);
      }

      else {
        if (!(edge->flag & 1))
          clear(list, ei, KisPathEdge::LeftTraversal);

        if (!(edge->flag & 2))
          traverse(list, ei, KisPathEdge::RightTraversal);
      }

      ++windingD;
    }
    // maybe when line is horizontal, i.e y0 == y1 ? -tanmay
    else {
      if (!(edge->flag & 1))
        clear(list, ei, KisPathEdge::LeftTraversal);
      ...
      std::cout << "curve rejected from add in handleCrossingEdges (else): "
                << edge->curveID << std::endl;
    }
  }
...
static void add(QPainterPath &path, const KisWingedEdge &list, int edge, KisPathEdge::Traversal traversal)
{
//    KisWingedEdge::TraversalStatus status;
//    status.edge = edge;
//    status.traversal = traversal;
    ...
    do {
        ...
        if ( ep->curveID == -1 ) { //ep->curveID == -1
            ...
            do {
                ...
                status = list.next(status);
                ep = list.edge(status.edge); // not const, will it make a difference? - tanmay
                addLineToProxy(path, *list.vertex(ep->vertex(status.direction)));
...
void KisWingedEdge::simplify()
{
    for (int i = 0; i < edgeCount(); ++i) {
        const KisPathEdge *ep = edge(i);
//        std::cout << "curve considered for simplify: " << ep->curveID << std::endl;
        // if both sides are part of the inside then we can collapse the edge
        int flag = 0x3 << 4;
        if ((ep->flag & flag) == flag) {
            removeEdge(i);
...
static void clear(KisWingedEdge& list, int edge, KisPathEdge::Traversal traversal)
{
    KisWingedEdge::TraversalStatus status;
    status.edge = edge;
    status.traversal = traversal;
...
QPainterPath KisPathClipper::clip(Operation operation)
{
    ...
    if (op != Simplify) {
        ...
        if (clipBounds.contains(subjectBounds)) {
            std::cout<<"clipBounds.contains(subjectBounds!!!!!!!!!!!!!!!!!!)"<<std::endl;   //tanmay
            if (clipIsRect) {
                switch (op) {
                case BoolSub:
                    return QPainterPath();
                case BoolAnd:
                    return subjectPath;
                case BoolOr:
                //clipPath.operationInClip = "clipbounds contains subjectbounds";  //tanmay
                    return clipPath;
                default:
                    break;
                }
            ...
            if (subjectIsRect) {
                ...
                case BoolAnd:
                    return clipPath;
                case BoolOr:
                    //subjectPath.operationInClip = "subjectbounds contains clipBounds";  //tanmay
                    return subjectPath;
                default:
                    break;
                }
...
bool KisPathClipper::doClip(KisWingedEdge &list, ClipperMode mode)
{
    ...
    do {
        ...
        for (int i = 0; i < list.edgeCount(); ++i) {
            KisPathEdge *edge = list.edge(i);
            std::cout << "                                               curve in doclip " << edge->curveID << std::endl;
...
static void traverse(KisWingedEdge &list, int edge, KisPathEdge::Traversal traversal)
{
    KisWingedEdge::TraversalStatus status;
    status.edge = edge;
    status.traversal = traversal;
...
static QVector<QCrossingEdge> findCrossings(const KisWingedEdge &list, qreal y)
{
    QVector<QCrossingEdge> crossings;
    for (int i = 0; i < list.edgeCount(); ++i) {
        const KisPathEdge *edge = list.edge(i);
...
bool KisPathClipper::handleCrossingEdges(KisWingedEdge &list, qreal y, ClipperMode mode)
{
    QVector<QCrossingEdge> crossings = findCrossings(list, y);

    Q_ASSERT(!crossings.isEmpty());
    std::sort(crossings.begin(), crossings.end()); // sorts all crossing edges according to their x co-ordinates
    ...
    qDebug() << "crossings:" << crossings.size();
#endif
    for (int i = 0; i < crossings.size() - 1; ++i) {  // windings nums ARE INCREMENTED INSIDE LOOP, CAN BE GREATER THAN 1
        int ei = crossings.at(i).edge;  // index of the edge having the crossing
        const KisPathEdge *edge = list.edge(ei);
        std::cout << "edge in handleCrossingEdges: " << edge->curveID << std::endl;
        ...
        // edge winding nums are incremented (only?) in intersectAndAdd, dependent on y-coords
        // windingA is changed only if pathId of edge is 0. otherwise B is changed. However this kinda doesn't make sense in the below code,
        // as windingB is related to bMask, which is clipShape's fill style.

        windingA += edge->windingA;
        windingB += edge->windingB;

        // Krita, in defaulttools.cpp, for union, used evenOdd filling. this implies aMask and bMask will always be 1,
        // and inA and inB's values are solely dependent on windingA and windingB.

        // if windingA is odd, inA will be true(1). if windingA is even, inA will be false(0). same for inB and windingB.
        const bool inA = (windingA & aMask) != 0; // aMask = subjectPath.fillRule() == Qt::WindingFill ? ~0x0 : 0x1;
        const bool inB = (windingB & bMask) != 0;

        const bool hasLeft = (edge->flag >> 4) & 1; // flag has 16 or 32 as value
        const bool hasRight = (edge->flag >> 4) & 2;

        const bool inside = bool_op(inA, inB, op); // decides to include or not by checking if it's inside A, inside B



        // ******* windingD *******

        windingD += hasLeft ^ hasRight; // windingD will be incremented by 1 if both values are different, else will remain same

        // on the line below, the last bit has to be 1. hence, windingD is supposed to be odd.
        const bool inD = (windingD & 0x1) != 0; // checks if winding num D (i.e edge hasLeft or hasRight, but not both  OVER ALL CROSSINGS) is 1
        ...
        if (add) {
            ...
            std::cout << "curve considered for add in handleCrossingEdges: " << edge->curveID << std::endl;
        ...
        // maybe when line is horizontal, i.e y0 == y1 ? -tanmay
            ...
            std::cout << "curve rejected from add in handleCrossingEdges (else): " << edge->curveID << std::endl;
        }
    }
...
static bool isVertical(Edge edge) { return edge == Left || edge == Right; }

template <Edge edge> bool compare(const QPointF &p, qreal t) {
  switch (edge) {
  case Left:
    return p.x() < t;
  case Right:
...
template <Edge edge>
void clipLine(const QPointF &a, const QPointF &b, qreal t,
              QPainterPath &result) {
...
template <Edge edge>
void clipBezier(const QPointF &a, const QPointF &b, const QPointF &c,
  ...
  QTransform flip =
      isVertical(edge) ? QTransform(0, 1, 1, 0, 0, 0) : QTransform();
  ...
  for (int i = 0; i < segmentCount; ++i) {
    ...
    if (outA != outB) {
      ...
      if (outB)
        addBezier(result,
                  unflipped.getSubRange(lastIntersection, intersection));
...
template <Edge edge> QPainterPath clip(const QPainterPath &path, qreal t) {
  QPainterPath result;
  for (int i = 1; i < path.elementCount(); ++i) {
    ...
    if (element.isLineTo()) {
      ...
      clipBezier<edge>(path.elementAt(i - 1), path.elementAt(i),
                       path.elementAt(i + 1), path.elementAt(i + 2), t, result);
      i += 2;
    }
  }
...
QPainterPath intersectPath(const QPainterPath &path, const QRectF &rect) {
  ...
  // The algorithm above might return one side of \a rect if there was no
  // intersection, so only return intersections that are not empty rectangles.
...
QPainterPath KisPathClipper::intersect(const QPainterPath &path,
                                       const QRectF &rect) {
...
QPainterPath KisPathClipper::intersect(const QPainterPath &path, const QRectF &rect)
{
    return intersectPath(path, rect);
>>>>>>> 974a07c7c1142b5af8c7c718bb0533b26c0637f5
}
